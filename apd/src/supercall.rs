...
use crate::package::{read_ap_package_config_validated, synchronize_package_uid};
...
pub fn refresh_ap_package_list(skey: &CStr, mutex: &Arc<Mutex<()>>) {
    // Poison-tolerant: a panicked holder must not permanently kill the
    // uid-listener thread; re-acquire the inner guard instead.
    let _lock = mutex.lock().unwrap_or_else(|e| e.into_inner());

    // Trim uninstalled packages from the config first so the grant/exclude
    // decisions below are made against the final persisted state.
    // Fail-open: on error the persisted config is left untouched and the
    // (possibly stale) snapshot below is still authoritative.
    if let Err(e) = synchronize_package_uid() {
        error!("Failed to synchronize package UIDs: {}", e);
    }

    // Read the config *before* touching the kernel allow list. If the file is
    // missing, truncated or being rewritten concurrently, reads can come back
    // empty even though grants still exist in the kernel. Revoking first and
    // then granting from an empty config would silently strip root from every
    // app until the next reboot, so bail out in that case.
    // `valid == false` means torn/unreadable; `valid && empty` means the user
    // legitimately revoked everything (header-only file) and revokes MUST
    // proceed.
    let read = read_ap_package_config_validated();
    let package_configs = read.configs;
    let config_valid = read.valid;
    ...
    // num == 0 means no grants in the kernel: nothing to revoke, just
    // (re-)grant below. This also skips sc_su_allow_uids, which rejects
    // empty buffers with -EINVAL.
    let num = num as usize;
    let mut uids: Vec<uid_t> = vec![0 as uid_t; num];
    if !uids.is_empty() {
        ...
        // The kernel returns the actual count; truncate padding zeros so the
        // grant-presence check and logs below see only real entries.
        let n = (n as usize).min(uids.len());
        uids.truncate(n);
    }

    let non_critical_grants = uids
        .iter()
        .filter(|uid| **uid != 0 && **uid != 2000)
        .count();
    // Only a *torn* read fuses the refresh. A valid-but-empty config is an
    // explicit "revoke everything" and must NOT be blocked.
    if !config_valid && non_critical_grants > 0 {
        error!(
            "[refresh_ap_package_list] package_config is unreadable but {} non-critical grant(s) exist \
             in the kernel; aborting refresh to avoid wiping every grant",
            non_critical_grants
        );
        return;
    }
    if !config_valid {
        warn!(
            "[refresh_ap_package_list] package_config unreadable, proceeding with {} parsed entr(ies)",
            package_configs.len()
        );
    }
