...
char HEADER_FILE_PATH[FILENAME_MAX] = {'\0'};
char KERNEL_FILE_PATH[FILENAME_MAX] = {'\0'};
char RAMDISK_FILE_PATH[FILENAME_MAX] = {'\0'};
char SECOND_FILE_PATH[FILENAME_MAX] = {'\0'};
char EXTRA_FILE_PATH[FILENAME_MAX] = {'\0'};
char KER_DTB_FILE_PATH[FILENAME_MAX] = {'\0'};
char RECV_DTBO_FILE_PATH[FILENAME_MAX] = {'\0'};
char DTB_FILE_PATH[FILENAME_MAX] = {'\0'};
char BOOT_PATH[FILENAME_MAX] = {'\0'};
char NEW_BOOT_PATH[FILENAME_MAX] = {'\0'};
...
void dyn_img_hdr::print() {
    ...
    if (char *checksum = id()) {
        fprintf(stdout, "%-*s [", PADDING, "CHECKSUM");
        for (int i = 0; i < SHA256_DIGEST_SIZE; ++i)
            fprintf(stdout, "%02hhx", checksum[i]);
        fprintf(stdout, "]\n");
    }
}

void dyn_img_hdr::dump_hdr_file() {
    FILE *fp = xfopen(HEADER_FILE_PATH, "w");
    if (name())
        fprintf(fp, "name=%s\n", name());
...
void dyn_img_hdr::load_hdr_file() {
    parse_prop_file(HEADER_FILE_PATH, [=](string_view key, string_view value) -> bool {
        if (key == "name" && name()) {
            memset(name(), 0, 16);
...
int split_image_dtb(const char *filename) {
    ...
    if (int off = find_dtb_offset(img.buf, img.sz); off > 0) {
        ...
        if (COMPRESSED(fmt)) {
            int fd = creat(KERNEL_FILE_PATH, 0644);
            decompress(fmt, fd, img.buf, off);
            close(fd);
        } else {
            dump(img.buf, off, KERNEL_FILE_PATH);
        }
        dump(img.buf + off, img.sz - off, KER_DTB_FILE_PATH);
        return 0;
    } else {
...
void working_file_path(const char *dir) {
    char sperate = 0;
    int len = strlen(dir);
    if(dir[len - 1] != '/')
        sperate = '/';

    strcat(HEADER_FILE_PATH, dir);
    HEADER_FILE_PATH[len] = sperate;
    strcat(HEADER_FILE_PATH, HEADER_FILE);

    strcat(KERNEL_FILE_PATH, dir);
    KERNEL_FILE_PATH[len] = sperate;
    strcat(KERNEL_FILE_PATH, KERNEL_FILE);

    strcat(RAMDISK_FILE_PATH, dir);
    RAMDISK_FILE_PATH[len] = sperate;
    strcat(RAMDISK_FILE_PATH, RAMDISK_FILE);

    strcat(SECOND_FILE_PATH, dir);
    SECOND_FILE_PATH[len] = sperate;
    strcat(SECOND_FILE_PATH, SECOND_FILE);

    strcat(EXTRA_FILE_PATH, dir);
    EXTRA_FILE_PATH[len] = sperate;
    strcat(EXTRA_FILE_PATH, EXTRA_FILE);

    strcat(KER_DTB_FILE_PATH, dir);
    KER_DTB_FILE_PATH[len] = sperate;
    strcat(KER_DTB_FILE_PATH, KER_DTB_FILE);

    strcat(RECV_DTBO_FILE_PATH, dir);
    RECV_DTBO_FILE_PATH[len] = sperate;
    strcat(RECV_DTBO_FILE_PATH, RECV_DTBO_FILE);

    strcat(DTB_FILE_PATH, dir);
    DTB_FILE_PATH[len] = sperate;
    strcat(DTB_FILE_PATH, DTB_FILE);

    strcat(BOOT_PATH, dir);
    BOOT_PATH[len] = sperate;
    strcat(BOOT_PATH, BOOT);

    strcat(NEW_BOOT_PATH, dir);
    NEW_BOOT_PATH[len] = sperate;
    strcat(NEW_BOOT_PATH, NEW_BOOT);
}

int unpack(const char *work_dir, bool skip_decomp, bool hdr) {
    working_file_path(work_dir);

    boot_img boot(BOOT_PATH);
    ...
    if (!skip_decomp && COMPRESSED(boot.k_fmt)) {
        ...
        dump(boot.kernel, boot.hdr->kernel_size(), KERNEL_FILE_PATH);
    }

    // Dump kernel_dtb
    dump(boot.kernel_dtb, boot.hdr->kernel_dt_size, KER_DTB_FILE_PATH);

    // Dump ramdisk
    if (!skip_decomp && COMPRESSED(boot.r_fmt)) {
        if (boot.hdr->ramdisk_size() != 0) {
            int fd = creat(RAMDISK_FILE_PATH, 0644);
            decompress(boot.r_fmt, fd, boot.ramdisk, boot.hdr->ramdisk_size());
            close(fd);
        }
    } else {
        dump(boot.ramdisk, boot.hdr->ramdisk_size(), RAMDISK_FILE_PATH);
    }

    // Dump second
    dump(boot.second, boot.hdr->second_size(), SECOND_FILE_PATH);

    // Dump extra
    if (!skip_decomp && COMPRESSED(boot.e_fmt)) {
        if (boot.hdr->extra_size() != 0) {
            int fd = creat(EXTRA_FILE_PATH, 0644);
            decompress(boot.e_fmt, fd, boot.extra, boot.hdr->extra_size());
            close(fd);
        }
    } else {
        dump(boot.extra, boot.hdr->extra_size(), EXTRA_FILE_PATH);
    }

    // Dump recovery_dtbo
    dump(boot.recovery_dtbo, boot.hdr->recovery_dtbo_size(), RECV_DTBO_FILE_PATH);

    // Dump dtb
    dump(boot.dtb, boot.hdr->dtb_size(), DTB_FILE_PATH);
...
void repack(const char *work_dir, const char *out_img, bool skip_comp) {
    working_file_path(work_dir);

    const boot_img boot(BOOT_PATH);
    fprintf(stdout, "Repack to boot image: [%s]\n", out_img);
    ...
    if (access(HEADER_FILE_PATH, R_OK) == 0)
        hdr->load_hdr_file();
    ...
    if (access(KERNEL_FILE_PATH, R_OK) == 0) {
        auto m = mmap_data(KERNEL_FILE_PATH);
    ...
    // kernel dtb
    if (access(KER_DTB_FILE_PATH, R_OK) == 0)
        hdr->kernel_size() += restore(fd, KER_DTB_FILE_PATH);
    file_align();
    ...
    if (access(RAMDISK_FILE_PATH, R_OK) == 0) {
        auto m = mmap_data(RAMDISK_FILE_PATH);
    ...
    if (access(SECOND_FILE_PATH, R_OK) == 0) {
        hdr->second_size() = restore(fd, SECOND_FILE_PATH);
        file_align();
    }

    // extra
    off.extra = lseek(fd, 0, SEEK_CUR);
    if (access(EXTRA_FILE_PATH, R_OK) == 0) {
        auto m = mmap_data(EXTRA_FILE_PATH);
    ...
    // recovery_dtbo
    if (access(RECV_DTBO_FILE_PATH, R_OK) == 0) {
        hdr->recovery_dtbo_offset() = lseek(fd, 0, SEEK_CUR);
        hdr->recovery_dtbo_size() = restore(fd, RECV_DTBO_FILE_PATH);
        file_align();
    }

    // dtb
    off.dtb = lseek(fd, 0, SEEK_CUR);
    if (access(DTB_FILE_PATH, R_OK) == 0) {
        hdr->dtb_size() = restore(fd, DTB_FILE_PATH);
        file_align();
    }
