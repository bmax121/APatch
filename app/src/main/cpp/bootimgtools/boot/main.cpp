...
static void usage(char *arg0) {
    fprintf(stderr,
R"EOF(
Andoird boot.img tools.

Copied and modified from Magisk. https://github.com/topjohnwu/Magisk.

Usage: %s <action> [args...]

Supported actions:
  unpack [-n] [-h] <workdir>
    Unpack boot.img in <workdir> to its individual components, each component to
    a file with its corresponding file name in <workdir>.
  ...
  repack [-n] <workdir> [outbootimg]
    Repack boot image components using files from <workdir>
    to [outbootimg], or 'new-boot.img' if not specified.
    <origbootimg> is the original boot image used to unpack the components.
    By default, each component will be automatically compressed using its
    corresponding format detected in boot.img which in <workdir>. If a component file
...
int main(int argc, char *argv[]) {
    ...
    if (action == "cleanup") {
        ...
        unlink(KERNEL_FILE_PATH);
        unlink(RAMDISK_FILE_PATH);
        unlink(SECOND_FILE_PATH);
        unlink(KER_DTB_FILE_PATH);
        unlink(EXTRA_FILE_PATH);
        unlink(RECV_DTBO_FILE_PATH);
        unlink(DTB_FILE_PATH);
        ...
        if (argv[2] == "-n"sv) {
            if (argc == 3)
                usage(argv[0]);
            repack(argv[3], argv[4] ? argv[4] : NEW_BOOT_PATH, true);
        } else {
            repack(argv[2], argv[3] ? argv[3] : NEW_BOOT_PATH);
        }
